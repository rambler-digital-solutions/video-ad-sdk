...
test('parseMacro must replace the passed variables in the macro', () => {
  ...
  expect(parseMacro(macro, {ERRORCODE: '101'})).toBe(
    'https://test.example.com/101/[BLA]'
  )
  expect(
    parseMacro(macro, {
      BLA: 'bla',
...
test('parseMacro must fill the CACHEBUSTING variable if not provided', () => {
  ...
  expect(parseMacro(macro, {})).toEqual(
    expect.stringMatching(/https:\/\/test.example.com\/\d+/)
  )
})
...
test('parseMacro must not fill the CACHEBUSTING variable if provided', () => {
  ...
  expect(parseMacro(macro, {CACHEBUSTING: 'foo'})).toBe(
...
test('parseMacro must percent-encode the replaced macros', () => {
  ...
  expect(parseMacro(macro, {URL: 'http://someExample.com'})).toBe(
    'https://test.example.com/http%3A%2F%2FsomeExample.com'
  )
})
...
test('parseMacro must not provide the TIMESTAMP variable if provided', () => {
  ...
  expect(parseMacro(macro, {TIMESTAMP: 'today'})).toBe(
    'https://test.example.com/today'
  )
})
