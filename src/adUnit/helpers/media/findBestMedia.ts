...
const getMediaByDefaultBestFit = (mediaFiles: MediaFile[], screenRect: ClientRect): MediaFile => {
  const sortedMediaFiles = sortMediaByBestFit(mediaFiles, screenRect);

  return sortedMediaFiles[0];
};

const findBestMedia = (inlineAd: ParsedAd, videoElement: HTMLVideoElement, container: HTMLElement, {getMediaFile = getMediaByDefaultBestFit}: Hooks): MediaFile | null => {
