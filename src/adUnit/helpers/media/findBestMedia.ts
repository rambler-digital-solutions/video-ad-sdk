...
const getMediaByDefaultBestFit = (mediaFiles: MediaFile[], screenRect: Screen): MediaFile => {
  const sortedMediaFiles = sortMediaByBestFit(mediaFiles, screenRect);

  return sortedMediaFiles[0];
};

const findBestMedia = (inlineAd: ParsedAd, videoElement: HTMLVideoElement, container: HTMLElement, {getMediaFile = getMediaByDefaultBestFit}): MediaFile | null => {
